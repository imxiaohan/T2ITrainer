...
本文档详细介绍了 `train_flux_lora_ui_with_mask.py` 训练脚本的所有参数，用于训练Flux模型的LoRA权重以进行图像填充（对象移除）任务。
...
| `--config_path` | JSON配置文件路径 | `config_fill.json` | 任意有效的配置文件路径 |
...
| `--mask_dropout` | 掩码丢弃比例，将掩码替换为全0 | `0.01` | 0-1之间的浮点数 |

### 训练控制参数

| 参数名 | 参数说明 | 默认值 | 可选值 |
|-----------|-------------|---------------|-----------------|
| `--resume_from_checkpoint` | 从检查点恢复训练 | `None` | 检查点路径或`null` |
...
| `--guidance_scale` | FLUX.1 dev变体的引导缩放因子，默认1以保持蒸馏 | `1` | 正浮点数 |
| `--reg_ratio` | 作为目标迁移学习的正则化。如果未训练不同目标，则设置为1 | `0.0` | 浮点数 |
