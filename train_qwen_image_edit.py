...
from utils.image_utils_qwenimage import CachedMutiImageDataset
...
import glob
import shutil
from utils.image_utils_qwenimage import compute_text_embeddings, replace_non_utf8_characters, create_empty_embedding, crop_image,get_md5_by_path,vae_encode
...
def main(args, config_args):
    ...
    def train_process(
        ...
        for image_config_key in image_configs.keys():
            # scale latents
            latent = batch[image_config_key][latent_key].to(device=accelerator.device,dtype=weight_dtype)
        ...
        final_caption = {
            ...
            prompt_embeds_mask_key : selected_caption[prompt_embeds_mask_key]
        }
        prompt_embeds = final_caption[prompt_embed_key].to(device=accelerator.device, dtype=weight_dtype)
        prompt_embeds_mask = final_caption[prompt_embeds_mask_key].to(device=accelerator.device, dtype=weight_dtype)
